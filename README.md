...
- **제어 센터형 사이드패널** — macOS 제어 센터처럼 홈은 위젯 격자 하나이고, 각 기능은
  위젯을 눌러 여는 시트에 담긴다. 위젯은 드래그로 옮기고 숨길 수 있으며,
  Ask AI 입력창은 하단에 상시 고정된다
- **Exploration Timeline** — 타임라인 시트. 날짜별 이벤트 스트림 + 세션 배지 +
  주간 탐색 분석, 세션 상세에서 탐색 경로를 시간순으로 복기
- **열린 탭 찾기** — Ask AI에 “아까 보던 영상으로 돌아가자”처럼 자연스럽게 입력하면
  열린 탭의 제목·호스트·경로를 의미 검색해 가장 가까운 탭으로 이동한다.
  열린 탭 시트에서는 창별 미리보기 카드로 훑어보고, 돋보기를 눌러 제목·주소를 검색할 수 있다.
  썸네일은 그 탭을 볼 때 로컬에서 캡처해 확장 저장소에만 두며 외부로 나가지 않는다
...
`identity`(구글 로그인).
`<all_urls>` 호스트 권한은 열린 탭 미리보기 썸네일(`chrome.tabs.captureVisibleTab`)에 쓰며,
캡처한 이미지는 확장 저장소에만 남고 전송하지 않습니다.
