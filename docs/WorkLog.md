...
설치→로그인→수집 opt-in→핵심 기능 이해 흐름을 구현했다.

- `orbit:onboarding` 상태를 추가하고 저장값이 없는 기존 사용자는 완료 상태로 간주한다.
- 최초 설치에서만 안내 탭을 열며, 로그인 후 사용자 CTA로 사이드패널을 연다.
- 첫 단계는 실제 `수집 켜기` 버튼으로 설정을 변경한다. mock은 API나 저장소에 쓰지 않는다.
- 실제 `CollectionOptInNotice`, `TimelineDateHeader`, `TimelineItem`, `SessionRow`와 열린 탭 행 형식을
  사용해 운영 화면과 같은 정보 밀도를 유지했다.
- 타임라인 10개, 세션 6개, 열린 탭 8개를 채우고 대표 행 여러 개를 함께 강조했다.
- 자동 화면 전환을 없애고 `세션`, `열린 탭`을 사용자가 직접 눌러야 다음 설명으로 진행하게 했다.
- 수집·타임라인·세션·열린 탭 설명은 강조 영역 아래, Ask 설명은 위에 배치했다.

**오류와 수정** — 최초 위치 측정을 다음 animation frame으로 미뤄 첫 프레임에 카드가 대상과
겹쳤다. layout effect에서 즉시 측정하고 좌표 준비 전에는 카드를 렌더링하지 않게 했다. mock 화면을
별도로 그려 실제 형식과 달랐던 부분은 기존 컴포넌트 재사용으로 교체했다. 탭이 자동으로 바뀌던
흐름은 탭 클릭 단계의 `다음` CTA를 제거하고 실제 탭 클릭 이벤트에만 전환을 연결했다.

**Playwright MCP 검증** — 빌드 확장을 headless Chromium에 로드해 다크 모드 461×799의 7단계
모두에서 카드와 강조 영역이 겹치지 않고 viewport 안에 있음을 확인했다. 수집·세션·열린 탭 버튼이
실제 포인터 입력을 받았고 직접 클릭 뒤 각각 onboarding step 1·3·5 저장을 확인했다. 320×700과
461×767에서도 첫 단계의 직접 클릭과 카드 배치를 확인했다.

**검증** — `pnpm.cmd test` 19개 파일 187개 통과 · `pnpm.cmd compile` 통과 ·
`pnpm.cmd build` 통과 · `git diff --check` 통과.

**미실시** — 실제 Google 계정 로그인과 브라우저 UI에서의 사이드패널 열기 통합 스모크.
...
## 2026-08-12 — 온보딩 완료 후 홈 복귀, 검색 기록 드롭다운 위치 수정
...
`replaceWithHome()`으로 새 탭 홈으로 돌아간다.

- `entrypoints/newtab/lib/navigation.ts` — `replaceWithHome()`, `isOnboardingLaunch()` 추가.
  `onboarding=1` 쿼리가 남으면 안내 화면이 계속 렌더되므로 쿼리까지 지우고 `replaceState` 로
  히스토리를 덮어쓴 뒤 popstate 를 알린다. 뒤로가기로 끝난 안내 화면에 돌아갈 이유가 없어
  pushState 를 쓰지 않았다.
- `entrypoints/newtab/components/sections/OnboardingLaunch.tsx` — 완료 감지와 이동.
- `entrypoints/newtab/main.tsx` — 안내 탭 여부를 렌더 중 URL 재독이 아니라 상태로 들고
  popstate 에서 갱신한다.

**처음 구현의 잘못된 목적지** — 첫 구현은 아틀라스(`#/orbit-atlas`)로 보냈다. 사용자 확인에서
첫 실행이 곧바로 세션 궤도 화면으로 떨어져 새 탭의 역할과 어긋난다는 지적을 받아 홈으로 바꿨다.

**홈으로 보낼 때만 생기는 함정** — 홈은 라우트 값이 그대로 `home` 이라 popstate 로 `setRoute`
를 호출해도 값이 같아 리렌더가 일어나지 않는다. 안내 탭 여부를 렌더 중에 `window.location`
에서 다시 읽는 기존 구조로는 안내 화면이 그대로 남는다. 그래서 `main.tsx` 에서 상태로 승격했다.
...
**미실시** — 실제 설치 흐름(설치 → 로그인 → 투어 완료 → 안내 탭 홈 복귀)의 브라우저 확인.
드롭다운 위치는 사용자 화면에서 문제가 확인됐고 수정 후 재확인은 사용자에게 남아 있다.

### 알려진 한계

- 안내 탭을 이미 닫은 상태에서 투어를 완료하면 돌려놓을 탭이 없어 사이드패널 전환만 일어난다.
  탭을 새로 열어 주는 동작은 사용자가 원하지 않은 탭을 만들 수 있어 넣지 않았다.
- 신규 사용자는 아직 세션이 없어 홈의 최근 탐색·추천이 빈 상태로 열린다.

---

## 2026-08-12 — 아틀라스 안내 배너 반응형 동작 수정

- 안내 배너가 문구의 본래 너비를 사용하되 캔버스 안에서만 줄바꿈되도록 수정했다.
- 닫기 버튼과 액션 영역의 축소를 막아 hover 중 클릭 영역이 찌그러지지 않게 했다.
- 캔버스 폭이 840px 이하이면 `다시 보지 않기`를 항상 표시해 hover 확장에 따른 재배치를 없앴다.
- 캔버스 폭이 720px 이하이면 글자와 간격을 소폭 줄여 한 줄을 유지하고, 실제로 공간이
