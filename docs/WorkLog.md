...
## 2026-08-08 — 사이드패널 제어 센터 개편 (feat/design)

macOS 제어 센터를 기준으로 사이드패널을 다시 만들었다. 여러 차례 시안 피드백을 받아
형태를 세 번 고쳤고, 마지막 형태만 남아 있다.

**구조**
- 상단 세그먼트 탭(타임라인 / 세션 / Ask AI)과 헤더를 없애고 **위젯 격자 + 시트 스택**
  단일 화면으로 합쳤다. `store/ui.ts` 의 `activeView`(뷰 5종)를 `sheets: Sheet[]` 스택으로 교체했다.
- `openSession()` 은 세션 목록 시트를 아래에 깔고 상세를 올린다. Ask 대화 중 관련 세션을
  열어도 Ask 시트가 스택에 남아 닫으면 대화로 돌아온다 — 단위 테스트로 고정했다.
- 새로고침·설정은 헤더에서 격자 안 아이콘 위젯으로 옮겼다. 다른 위젯처럼 옮기고 숨길 수 있다.

**위젯**
- 크기 세 가지(`wide` 2×1 알약 / `small` 1×1 원 / `full` 4×1 스트립).
- 상태 표시를 macOS 규칙으로 맞췄다 — 켜짐은 흰 원 + 액센트 글리프, 꺼짐은 회색. 토글
  스위치를 없애고 타일 자체를 눌러 켜고 끈다. 흰 카드 위의 흰 디스크는 보이지 않으므로
  타일 배경을 배경보다 한 단계 눌러 깐 토큰(`--color-orbit-tile`)을 새로 뒀다.
- 타일에서 곁가지 정보를 걷어냈다(방문/미처리 수치, 파비콘 줄, 추천 종류 배지 등).
  상태는 `켬`·`11개`처럼 최소 단어만 남겼다.
- 테두리를 없애고 배경색과 그림자로만 구분해, 좁은 폭에서 윤곽선이 잘려 보이던 문제를 없앴다.
- 배치 편집: 드래그 재정렬 · `−` 숨기기 · `+` 되돌리기. `chrome.storage.local` 에 저장하며,
  `normalizeLayout()` 이 모르는 id 를 버리고 새로 생긴 위젯을 뒤에 붙여 버전이 올라가도
  기존 배치를 잃지 않는다.
- 추가한 위젯: 추천 세션(12초 회전), 오늘의 탐색(시간대별 막대), 최다 도메인(기본 숨김),
  대시보드 열기.

**시트**
- 격자 위에 뜨는 **흰 패널**로 바꾸고 닫기 `×` 를 오른쪽 끝 작은 버튼으로 옮겼다. 패널이
  흰색이 되면서 안쪽 흰 카드가 묻혀, 타임라인 분석 카드 · Ask 답변 말풍선 · 설정 그룹 ·
  병합 카드 · 배지 · hover 를 타일 색으로 함께 옮겼다.
- **세션 상세를 시트에서 목록 안 인라인 펼침으로 바꿨다.** `{kind:'session'}` 시트를 없애고
  `expandedSessionId` 로 대체했다. 하나를 고르면 다른 세션과 구역 제목이 접혀 고른 행이
  맨 위로 올라오고 상세가 아래로 흘러내린다. 시트를 닫으면 펼침도 함께 접힌다 —
  남겨 두면 다음에 목록을 열었을 때 엉뚱한 세션이 펼쳐진 채로 뜬다(테스트로 고정).
  높이 전환은 `max-height` 대신 grid `0fr → 1fr`(`.orbit-collapse`)을 써서 내용 길이와
  무관하게 속도를 같게 했다.
- 목록 형식을 macOS Wi-Fi 팝오버 행으로 통일했다 — 구역 제목 + `원형 디스크 · 이름 한 줄 ·
  오른쪽 보조 표시`. 저장된 세션(오늘/지난 7일/이전), 세션 상세의 탭 목록, Ask 답변의 관련
  세션이 모두 같은 형식이다. 탭 목록에서 URL 두 줄 표시를 없앴다(툴팁·링크 복사로 대체).
- 열린 탭 시트는 창별 2열 **미리보기 카드 그리드**가 됐다.

**열린 탭 미리보기**
- 처음에는 사이드패널 왼쪽(페이지 위)에 콘텐츠 스크립트 오버레이로 띄우려 했으나 동작하지
  않았다. 브라우저에서 직접 확인해 DOM·CSS 는 정상임을 확인했고(렌더 위치 x=1710, 286×994),
  원인은 주입 경로였다. 사용자 결정으로 오버레이를 걷어내고 사이드패널 안으로 옮겼다.
  `entrypoints/tab-preview.content.ts` 와 `scripting` 권한을 되돌렸다.
- 썸네일은 백그라운드가 활성 탭만 조용히 찍어(`captureVisibleTab`) 320px JPEG 로 줄여
  저장한다. 최근 24개만 유지하고 탭이 닫히면 지운다. 주소가 바뀐 탭은 옛 썸네일을 쓰지 않는다.
- 아직 안 찍힌 탭은 도메인에서 뽑은 색으로 표지 카드를 그린다.

**정리**
- 삭제: `TopNavBar`, `OpenTabsPanel`, `CurrentSessionCard`, `SyncStatusCard`, `SessionCard`,
  `SessionRow`, `ControlTile`, `ToggleSwitch`, `SidePanelHeader`,
  `views/{TimelineView,SessionListView,SearchView,SessionDetailView}`.
  `SettingsView` 는 `SettingsSheet` 로 이동(내용 변경 없음).
- 북마크 기능 제거: `bookmarkOpenTabs`, `uniqueBookmarkTabs`, `isBookmarkableUrl`,
  `OpenTabItem.bookmarkable`, manifest `bookmarks` 권한, 관련 테스트. 타입을 먼저 좁혀
  UI 에서 빠뜨린 참조가 없도록 했다.
- Ask 입력창의 좌측 `✦` 버튼을 없앴다.

**자체 검토에서 잡은 것**
- 시트가 격자를 덮기만 하고 언마운트하지 않아 포커스가 뒤로 새던 문제 → 격자에 `inert`.
- `prefers-reduced-motion` 에서 시트·타일·편집 흔들림 애니메이션을 멈추도록 했다.

**검증**
- extension **128 passed (15 files)**, `tsc --noEmit` 통과, Chrome MV3 빌드 통과(841.83 kB).
- 검증 중 `engine.test.ts` 가 한 번 실패했다. 이번 변경과 무관한 기존 경쟁 조건으로,
  `synced` 개수만 기다리고 그 뒤에 일어나는 `serverSync` 호출은 기다리지 않아 부하가 걸릴 때
  아직 호출 전 상태에서 단정했다. 대기 조건에 그 호출을 포함시켜 5회 연속 통과를 확인했다.
- 실제 크롬 로드 시각 확인은 사용자 테스트로 남겼다.
