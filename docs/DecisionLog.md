...
| 노이즈 사전 필터 | 체류 임계(예: 30초) 미만 + SNS/광고성 도메인 이벤트를 LLM 무호출로 discard하는 서버측 결정 규칙 도입 여부 | 임계값·도메인 목록 정의, 데이터 처리 정책 합의. **우선순위 상향(2026-08-05)** — 프롬프트 보강 2건이 모두 회귀를 유발해 반려됐고 A.X-K1 비결정성으로 스침 방문 혼입이 잔존(위 "프롬프트 추가 보강 반려" 항목). hold 누적이 강제 create로 빠지는 정책도 함께 재검토 필요 | 미해결 |
