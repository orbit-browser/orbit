...
**상태:** 제안 (2026-08-06) — 설계만. 구현은 별도 승인 후.
...
**비목표**
* **자동 병합 금지** — 기존 제품 원칙(improvement-report, ProjectContext) + AGENTS §11(AI 파괴적 자동
  실행 금지)에 따라 사용자 확인 없이 세션을 합치지 않는다. 시스템은 **제안까지만** 자동으로 한다.
...
4. **P3 undo**.
5. **P4(선택)** — 배치 후 제안 자동 생성 + 사이드패널/대시보드 뱃지 노출.

## 9. 위험 / 사용자 결정 필요

| 항목 | 선택지 | 기본 추천 |
|---|---|---|
| 자동 vs 제안 | (확정) 제안+확인, 자동 금지 | — |
| 후보 임계값 | `merge_suggest_floor` 값, 벡터만 vs 벡터+겹침 필수 | 벡터+키워드 겹침 AND, floor 0.6~0.7 (튜닝) |
| 생존 세션 선택 | 이벤트 많은 쪽 / 먼저 만든 쪽 | 이벤트 많은 쪽(동률 시 이른 started_at) |
| 삭제 방식 | soft(status=merged, 가역) / hard | soft-delete + undo |
| 제안 노출 시점 | 배치 후 자동 생성 / 수동 요청만 | P1은 수동, P4에서 자동 노출 |
| UI 위치 | 사이드패널 / 웹 대시보드 / 둘 다 | 대시보드 우선(검토 화면) |

## 10. 왜 지금 구현하지 않는가

병합은 **데이터 모델 변경 + 파괴적(가역) 연산 + 제품 UX 흐름**이라 AGENTS §4의 사용자 결정 영역이 크다.
서브클러스터링/게이팅으로 급한 불(메가 뭉침)은 껐으므로, 병합은 위 열린 결정을 사용자와 합의한 뒤
P1(읽기 전용 제안)부터 안전하게 착수하는 것이 옳다.
